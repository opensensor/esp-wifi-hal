#!/usr/bin/env python3
"""Re-extract code and data from the pinned pre-RF-PLL ELF and linker map.
...
code.NAMES={c:[f['name'] for f in e['functions']] for c,e in LAYOUT.items()}
code.HELPERS={c:[n for n in e['symbols'] if n not in ('phy_param','g_phyFuns')] for c,e in LAYOUT.items()}
...
def extract(chip, path, map_path, objdump):
    ...
    result['constants']=[{**row,'bytes':read(int(row['address'],0),len(bytes.fromhex(row['bytes']))).hex()} for row in layout['constants']]

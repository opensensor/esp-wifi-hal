...
import audit_phy_txiq_wrappers as audit
from test_audit_phy_tx_iq_measure import fixture as detector_fixture
...
def fixture(chip='esp32s3', expected='source'):
    ...
    if expected == 'source':
        for i, (old, new) in enumerate(audit.SELECTED.items()):
            symbols[old] = symbol(0x42007000 + i * 64)
            symbols[new] = symbol(0x42007000 + i * 64)
...
def check(base, symbols, expected='source'):
    audit.previous.previous.check_detector(base, symbols, 'source', expected_iq='source', expected_wrappers=expected)
...
class Tests(unittest.TestCase):
    ...
    def test_default_detector_gate_still_requires_vendor_iq(self):
        with self.assertRaises(ValueError):
            audit.previous.previous.check_detector(*fixture(), 'source', expected_iq='source')
    ...
    def test_original_text_literals_and_overlap_rejected(self):
        for name in audit.SELECTED:
            for prefix in ('.text.', '.literal.'):
                ...
                with self.assertRaises(ValueError):
                    check(b, s)
        b, s = fixture(); append_input(b, 'wrong.o', '.text', 0x42007001, 12)
        with self.assertRaises(ValueError):
            check(b, s)
    ...
    def test_vendor_control_owner_and_absent_source(self):
        ...
        for row in b['allocations']['libphy.a']['inputs']:
            if row['section'] == '.text.txiq_cal_init':
                row['member'] = 'wrong.o'
        with self.assertRaises(ValueError):
            check(b, s, 'vendor')
        b, s = fixture(expected='vendor'); s['__opensensor_txiq_initialize'] = symbol(0x42009000)
        with self.assertRaises(ValueError):
            check(b, s, 'vendor')

    def test_all_twelve_remaining_vendor_bodies_required(self):
        self.assertEqual(len(audit.RETAINED), 12)
        for name in audit.RETAINED:
            b, s = fixture(); s[name] = None
            with self.assertRaises(ValueError):
                check(b, s)

    def test_prior_iq_body_cannot_be_dropped(self):
        b, s = fixture(); s['__opensensor_txiq_measure'] = None
        with self.assertRaises(ValueError):
            check(b, s)
    ...
    def test_earlier_gates_cannot_be_skipped_without_elftools(self):
        with patch.dict(sys.modules, {'elftools': None, 'elftools.elf': None,
                ...
                audit.previous.previous.allocations, 'audit', return_value={}), patch.object(
                audit.previous.previous.previous, 'audit', side_effect=ValueError('earlier ownership failed')):

...
def extract(chip, path, objdump, baseline):
    NAMES[chip] = baseline['functions']
    HELPERS[chip] = baseline['helpers']
    raw = path.read_bytes()
...
def main():
    ...
    p.add_argument('chip', choices=('esp32c3','esp32s3'))
    p.add_argument('elf', type=Path)
    p.add_argument('objdump')
    p.add_argument('output', type=Path)
    args = p.parse_args()
    baseline = json.loads(Path(__file__).with_name(args.chip+'-baseline.json').read_text())

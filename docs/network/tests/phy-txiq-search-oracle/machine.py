    ...
    def execute(self,pc,r,kind,depth=0):
        ...
        try:
            while True:
                ...
                if op=='entry':
                ...
                elif op in ('add','add.n','addx2','addx4','addx8','mul','sub','and','or','xor','div','mull','mulsh','sltu','slt','subx2','subx4','subx8','mulhu','mulh','max','min','rem','remu','divu','muluh','minu','maxu','quos'):
                    ...
                    if op in ('add','add.n'):value=left+right
                        ...
                        machine_left,machine_right=signed(left),signed(right);require(s3 and machine_right!=0,'Unmodeled Xtensa divisor or ABI')
                        value=(abs(machine_left)//abs(machine_right))*(-1 if (machine_left<0)!=(machine_right<0) else 1)
                    else:

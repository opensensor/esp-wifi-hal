...
def check_iq(base,symbols,expected):
...
def audit(elf_path,map_path,label,expected):
 ...
 prior=previous.audit(elf_path,map_path,label,'source',iq_source=(expected=='source'));base=allocations.audit(elf_path,map_path,label,exclude_strings=True)
 ...
 check_iq(base,symbols,expected)

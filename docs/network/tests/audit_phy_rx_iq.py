...
def check_iq(base,symbols,expected,rf_iq_source=False,rx_dc_source=False,dc_search_source=False,rx_gain_cal_source=False,spur_source=False):
 ...
 if not member and not ((rx_gain_cal_source and base['chip']=='esp32c3') or spur_source):raise ValueError('Calibration search member unexpectedly absent')
...
def audit(elf_path,map_path,label,expected,rf_iq_source=False,rx_dc_source=False,dc_search_source=False,rx_gain_cal_source=False,spur_source=False):
 from elftools.elf.elffile import ELFFile
 prior=previous.audit(elf_path,map_path,label,'source',iq_source=(expected=='source'),rf_iq_source=rf_iq_source,rx_dc_source=rx_dc_source,dc_search_source=dc_search_source,rx_gain_cal_source=rx_gain_cal_source,spur_source=spur_source);base=allocations.audit(elf_path,map_path,label,exclude_strings=True)
 ...
 check_iq(base,symbols,expected,rf_iq_source,rx_dc_source,dc_search_source,rx_gain_cal_source=rx_gain_cal_source,spur_source=spur_source)

...
class IntegerDivideByZero(ArithmeticError):
    """Xtensa EXCCAUSE 6; instruction destination is not written."""
    exccause = 6
        ...
        try:
            while True:
                ...
                if op=='entry':
                    ...
                    require(s3, 'QUOS requires Xtensa')
                    left,right=signed(r[args[1]]),signed(r[args[2]])
                    if right==0:raise IntegerDivideByZero('Xtensa integer divide by zero')
                    quotient=(abs(left)//abs(right))*(-1 if (left<0)!=(right<0) else 1)
                    r[args[0]]=quotient&MASK

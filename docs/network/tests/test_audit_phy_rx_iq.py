import unittest
import audit_phy_rx_iq as audit
...
class Tests(unittest.TestCase):
 def test_both_stages(self):
  for chip in ('esp32c3','esp32s3'):
   for mode in ('source','vendor'):audit.check_iq(*fixture(chip,mode),mode)
 def test_unknown_stage(self):
  with self.assertRaises(ValueError):audit.check_iq(*fixture(),'auto')
 def test_aliases(self):
  for name in audit.SELECTED:
   b,s=fixture();s[name]['address']='0x42000000'
   with self.assertRaises(ValueError):audit.check_iq(b,s,'source')
 ...
 def test_vendor_cannot_have_source(self):
  ...
  with self.assertRaises(ValueError):audit.check_iq(b,s,'vendor')

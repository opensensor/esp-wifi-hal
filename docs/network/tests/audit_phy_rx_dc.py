...
def check_rx_dc(base,symbols,expected):
...
def audit(elf_path,map_path,label,expected):
 ...
 prior=previous.audit(elf_path,map_path,label,'source',rx_dc_source=(expected=='source'));base=allocations.audit(elf_path,map_path,label,exclude_strings=True)
 ...
 check_rx_dc(base,symbols,expected)

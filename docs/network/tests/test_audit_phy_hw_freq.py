import hashlib
import unittest
import audit_phy_hw_freq as track
...
def fixture(chip='esp32c3',expected='source'):
    ...
    for i,(old,new) in enumerate(track.SELECTED[chip].items()):
        ...
        if expected=='vendor':append_input(base,'phy_hw_freq.o','.iram1' if new in track.IRAM else '.text.'+old,address,32)
    return base,syms
class HardwareFrequencyAudit(unittest.TestCase):
    def test_both_modes_and_chips(self):
        for chip in track.SELECTED:
            for mode in ['vendor','source']:track.check_hw_freq(*fixture(chip,mode),mode)
    def test_unknown_mode(self):
        with self.assertRaises(ValueError):track.check_hw_freq(*fixture(),'automatic')
    def test_every_member_input_rejected(self):
        for section in ['.iram1','.rodata','.data','COMMON','.text.other','.literal.get_power_db']:
            b,s=fixture();append_input(b,'phy_hw_freq.o',section)
            with self.assertRaisesRegex(ValueError,'still has allocated'):track.check_hw_freq(b,s,'source')
    def test_excluded_strings_rejected(self):
        b,s=fixture();b['excluded_mergeable_string_inputs']=[{'member':'phy_hw_freq.o','reported_input_bytes':1}]
        with self.assertRaisesRegex(ValueError,'excluded mergeable'):track.check_hw_freq(b,s,'source')
    def test_every_alias_checked(self):
        for chip in track.SELECTED:
            for old in track.SELECTED[chip]:
                b,s=fixture(chip);s[old]['address']=hex(int(s[old]['address'],0)^8)
                with self.assertRaisesRegex(ValueError,'Incorrect hardware frequency source alias'):track.check_hw_freq(b,s,'source')
    def test_stale_alias_sizes_are_not_body_sizes(self):
        for chip in track.SELECTED:
            ...
            track.check_hw_freq(b,s,'source')
        ...
        for chip in track.SELECTED:
            for name in track.SELECTED[chip].values():
                for field,value in [('allocated',False),('body_contained',False),('symbol_size_bytes',0),('type','STT_OBJECT'),('executable',False)]:
                    ...
                    with self.assertRaisesRegex(ValueError,'Missing allocated function'):track.check_hw_freq(b,s,'source')
    def test_partial_vendor_overlap_rejected(self):
        b,s=fixture();append_input(b,'other.o','.data',0x40380008)
        with self.assertRaisesRegex(ValueError,'overlaps vendor'):track.check_hw_freq(b,s,'source')
    def test_old_named_function_under_other_member_rejected(self):
        for name in track.SELECTED['esp32c3']:
            ...
            with self.assertRaisesRegex(ValueError,'Original hardware frequency function'):track.check_hw_freq(b,s,'source')
    def test_vendor_needs_correct_member_and_section(self):
        for field,value in [('member','other.o'),('section','.text.wrong')]:
            ...
            with self.assertRaisesRegex(ValueError,'lacks member ownership'):track.check_hw_freq(b,s,'vendor')
    def test_vendor_rejects_every_source_symbol(self):
        for chip in track.SELECTED:
            for name in track.ALL_SOURCE_NAMES:
                ...
                with self.assertRaisesRegex(ValueError,'Unexpected hardware frequency source'):track.check_hw_freq(b,s,'vendor')
    def test_missing_alias_and_missing_body_rejected(self):
        for chip in track.SELECTED:
            for old,new in track.SELECTED[chip].items():
                ...
                with self.assertRaisesRegex(ValueError,'Missing allocated function'):track.check_hw_freq(b,s,'source')
    def test_all_three_source_helpers_must_stay_in_iram(self):
        for chip in track.SELECTED:
            for old,new in track.SELECTED[chip].items():
                ...
                for address in [0x42010000,0x40000000,0x3fc80000,0x403ffff8]:
                    ...
                    with self.assertRaisesRegex(ValueError,'must reside in IRAM'):track.check_hw_freq(b,s,'source')

"""Check RX gain-calibration source ownership while retaining calibration search bodies."""
...
def check_rx_gain_cal(base,symbols,expected):
 ...
 if expected=='source' and base['chip']=='esp32c3':
  if member:raise ValueError('C3 RX calibration member still allocated')
  if any(r['member']=='phy_rx_cal.o' and r['reported_input_bytes'] for r in base.get('excluded_mergeable_string_inputs',[])):raise ValueError('C3 RX calibration strings still allocated')
...
def audit(elf_path,map_path,label,expected):
 ...
 prior=previous.audit(elf_path,map_path,label,'source',rx_gain_cal_source=(expected=='source'));base=allocations.audit(elf_path,map_path,label,exclude_strings=True)
 ...
 check_rx_gain_cal(base,symbols,expected)

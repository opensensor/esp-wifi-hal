import unittest
import audit_phy_dc_search as audit
...
class Tests(unittest.TestCase):
 def test_prior_gates_require_explicit_transition(self):
  import audit_phy_rx_dc as dc, audit_phy_rf_iq as rf, audit_phy_rx_iq as iq, audit_phy_rx_controls as controls
  from test_audit_phy_rx_dc import fixture as dc_fixture
  ...
  for module,make,check in [(dc,dc_fixture,dc.check_rx_dc),(rf,rf_fixture,rf.check_rf_iq),(iq,iq_fixture,iq.check_iq),(controls,controls_fixture,controls.check_controls)]:
   for chip in ('esp32c3','esp32s3'):
    ...
    check(b,s,'source',dc_search_source=True)
 def test_transition_preserves_remaining_bodies(self):
  import audit_phy_rx_dc as dc
  from test_audit_phy_rx_dc import fixture as make
  for chip in ('esp32c3','esp32s3'):
   for name in audit.RETAINED[chip]:
    b,s=make(chip);s[name]['address']='0x42009900'
    with self.assertRaises(ValueError):dc.check_rx_dc(b,s,'source',dc_search_source=True)
 def test_both_stages(self):
  for chip in ('esp32c3','esp32s3'):
   for mode in ('source','vendor'):audit.check_dc_search(*fixture(chip,mode),mode)
 def test_unknown_stage(self):
  with self.assertRaises(ValueError):audit.check_dc_search(*fixture(),'auto')
 def test_aliases(self):
  for name in audit.SELECTED['esp32c3']:
   b,s=fixture();s[name]['address']='0x42000000'
   with self.assertRaises(ValueError):audit.check_dc_search(b,s,'source')
 ...
 def test_vendor_cannot_have_source(self):
  ...
  with self.assertRaises(ValueError):audit.check_dc_search(b,s,'vendor')

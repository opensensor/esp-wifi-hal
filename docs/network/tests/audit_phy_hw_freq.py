...
def audit(elf_path,map_path,label,expected,*,reg_source=False,tx_gain_source=False,init_source=False):
    from elftools.elf.elffile import ELFFile
    if not re.fullmatch(r'[A-Za-z0-9_.-]+',label):raise ValueError('Label must be a simple artifact identifier')
    prior=previous.audit(elf_path,map_path,label,'source',hw_freq_source=(expected=='source'),reg_source=reg_source,tx_gain_source=tx_gain_source,init_source=init_source)
